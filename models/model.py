...
class PreNorm_cross(nn.Module):
    ...
    def forward(self, x, y, **kwargs):
        return self.fn(self.norm_1(x), self.norm_2(y) , **kwargs)
...
class Attention_4(nn.Module):
    ...
    def forward(self, x):
        h = self.heads
        q, k, v = self.to_qkv(x).chunk(3, dim = -1)
        q, k, v = map(lambda t: rearrange(t, 'b c n (h d) -> b c h n d', h = h), (q, k, v))
        sim = einsum('b c h i d, b c h j d -> b c h i j', q, k) * self.scale
        attn = sim.softmax(dim = -1)
...
class Attention_Cross(nn.Module):
    ...
    def forward(self, x, y, mask = None):
        ...
        q, k, v = map(lambda t: rearrange(t, 'b c n (h d) -> b c h n d', h = h), (q, k, v))
        sim = einsum('b q h i d, b k h j d -> b q h i j', q, k) * self.scale
        attn = sim.softmax(dim = -1)
        out = einsum('b q h i j, b k h j d -> b q h i d', attn, v)
        out = rearrange(out, 'b q h n d -> b q n (h d)', h=h)
        return self.to_out(out)
...
class Fusion_layer(nn.Module):
    def __init__(self, num_tokens, dim, nfeats, depth, heads, dim_head, attn_dropout, ff_dropout,style='col'):
        ...
        for _ in range(depth):
            if self.style == 'colrow':
                self.layers.append(nn.ModuleList([
                    PreNorm_cross(dim, Residual_Cross(Attention_Cross(dim, heads = heads, dim_head = dim_head, dropout = attn_dropout))),
                    PreNorm(dim, Residual(FeedForward(dim, dropout = ff_dropout))),
                    # PreNorm(dim*nfeats, Residual(Attention_Cross(dim*nfeats, heads = heads, dim_head = 64, dropout = attn_dropout))),
                    # PreNorm(dim*nfeats, Residual(FeedForward(dim*nfeats, dropout = ff_dropout))),
                ]))
            else:
                self.layers.append(nn.ModuleList([
                    PreNorm_cross(dim*nfeats, Residual_Cross(Attention_Cross(dim*nfeats, heads = heads, dim_head = 64, dropout = attn_dropout))),
                    PreNorm(dim*nfeats, Residual(FeedForward(dim*nfeats, dropout = ff_dropout))),
                ]))
    ...
    def forward(self, x, y, mask = None):
        ...
        if self.style == 'colrow':
            for attn1, ff1 in self.layers: 
             ...
             for attn1, ff1 in self.layers:
                x = rearrange(x, 'b c n d -> 1 b c (n d)')
                x = attn1(x,y)
                x = ff1(x)
                x = rearrange(x, '1 b c (n d) -> b c n d', n = n)
        return x




class RowColTransformer_1(nn.Module):
    def __init__(self, num_tokens, dim, nfeats, depth, heads, dim_head, attn_dropout, ff_dropout,style='col'):
        super().__init__()
        self.embeds = nn.Embedding(num_tokens, dim)
        self.layers = nn.ModuleList([])
        ...
        for _ in range(depth):
            if self.style == 'colrow':
                self.layers.append(nn.ModuleList([
                    PreNorm(dim, Residual(Attention_4(dim, heads = heads, dim_head = dim_head, dropout = attn_dropout))),
                    PreNorm(dim, Residual(FeedForward(dim, dropout = ff_dropout))),
                    PreNorm(dim*nfeats, Residual(Attention_4(dim*nfeats, heads = heads, dim_head = 64, dropout = attn_dropout))),
                    PreNorm(dim*nfeats, Residual(FeedForward(dim*nfeats, dropout = ff_dropout))),
                ]))
            else:
                self.layers.append(nn.ModuleList([
