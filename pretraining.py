...
from augmentations import embed_data_mask, embed_data_dict
...
def count_loss(dataloader, model, opt, device, criterion1, criterion2):
    model.eval()
    ...
    loss_aver = loss_total/j
    return loss_aver

def pretrain_process(model,trainloader,validloader, opt, device, model_save_path):
    ...
    for epoch in range(opt.pretrain_epochs):
        ...
        if epoch%5==0:
            loss = count_loss(validloader, model, opt, device, criterion1, criterion2)
            if loss < threshold:
                torch.save(model.state_dict(),f'{model_save_path}/{epoch}_{loss}.pth')
                print(f'model with loss: {loss} saved ')

    print('END OF PRETRAINING!')
    return model

def pretrain_process_dict(model,trainloader,validloader, opt, device, model_save_path):
    ...
    for epoch in range(opt.pretrain_epochs):
        ...
        for i, data in enumerate(trainloader, 0):
            optimizer.zero_grad()
            x_categ, x_cont,cat_mask, con_mask, key, time_stamp = data[0].to(device), data[1].to(device),data[2].to(device),data[3].to(device), data[4].to(device), data[5].to(device)
            
            # embed_data_mask function is used to embed both categorical and continuous data.
            if 'cutmix' in opt.pt_aug:
                from augmentations import add_noise
                x_categ_corr, x_cont_corr = add_noise(x_categ,x_cont, noise_params = pt_aug_dict)
                _ , x_categ_enc_2, x_cont_enc_2 = embed_data_dict(x_categ_corr, x_cont_corr, cat_mask, con_mask, model,key,time_stamp,vision_dset)
            else:
                _ , x_categ_enc_2, x_cont_enc_2 = embed_data_dict(x_categ, x_cont, cat_mask, con_mask,model,key,time_stamp,vision_dset)
            _ , x_categ_enc, x_cont_enc = embed_data_dict(x_categ, x_cont, cat_mask, con_mask,model,key,time_stamp,vision_dset)
